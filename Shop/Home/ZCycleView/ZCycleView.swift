...
public class ZCycleView: UIView {
    ...
    /// 设置网络图片
    /// - Parameter urlsGroup: url数组
    /// - Parameter group: 内容数组
    public func setGroupList(_ groupList: [[String: AnyObject]] = []) {
        if groupList.count == 0 { return }
        realDataCount = groupList.count
    ...
    private var reuseIdentifier1: String = "CaseBannerCollectionViewCell"
    ...
    private var groupList: [[String: AnyObject]] = []
...
extension ZCycleView {
    ...
    private func registerCell() {
        ...
        if let customClass = delegate?.cycleViewCustomCellClass?() {
            ...
            collectionView.register(CaseBannerCollectionViewCell.self, forCellWithReuseIdentifier: reuseIdentifier1)
        }
    }
...
extension ZCycleView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if type == 1 {
            let cycleCell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier1, for: indexPath)
            let index = indexPath.item % realDataCount
            
            if self.delegate?.cycleViewCustomCellSetup?(self, cell: cycleCell, for: index) != nil {
                return cycleCell
            }
            guard let cell = cycleCell as? CaseBannerCollectionViewCell else { return cycleCell }
            let imageUrl = index < groupList.count ? (groupList[index]["img"] as? String ?? "") : nil
            delegate?.cycleViewConfigureDefaultCellImageUrl?(self, imageView: cell.img, imageUrl: imageUrl, index: index)
            if delegate?.cycleViewConfigureDefaultCellImageUrl?(self, imageView: cell.img, imageUrl: imageUrl, index: index) == nil {
